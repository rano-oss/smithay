...
use crate::wayland::input_method_v3::InputMethodManagerState;
...
use wl_input_method::input_method::v1::server::xx_input_method_keyboard_v1::{
    self, FilterAction, XxInputMethodKeyboardV1,
};
...
impl KeyboardEvent {
    ...
    fn apply<T: WlKeyboardApi + ?Sized>(&self, k: &T) {
        ...
        match self {
            KeyboardEvent::Keymap { format, fd, size } => k.keymap(*format, fd.as_fd(), *size),
            KeyboardEvent::Enter {
                serial,
                surface,
                keys,
            } => k.enter(*serial, surface, keys.clone()),
            KeyboardEvent::Leave { serial, surface } => k.leave(*serial, surface),
            KeyboardEvent::Key {
                serial,
                time,
                key,
            ...
            KeyboardEvent::RepeatInfo { rate, delay } => k.repeat_info(*rate, *delay),
        }
    }
...
impl KeyFilter {
    ...
    pub(crate) fn create(
        ...
        client_keyboards: &Arc<Mutex<Vec<wayland_server::Weak<wl_keyboard::WlKeyboard>>>>,
        surface: &WlSurface,
    ) -> Self {
        let ret = KeyFilter {
    ...
    fn forward(&self, event: &KeyboardEvent) {
        ...
        KnownKbds::for_each_focused_kbd(&*keyboards.lock().unwrap(), &self.focused_surface, |k| {
            event.apply(k)
        });
    }
}
...
impl WlKeyboardApi for KeyFilter {
    ...
    fn enter(&self, serial: u32, surface: &wl_surface::WlSurface, keys: Vec<u8>) {
        self.keyboard
            .im_keyboard
            .enter(serial, &self.keyboard.im_surface, keys.clone());
    ...
    fn key(&self, serial: u32, time: u32, key: u32, state: wl_keyboard::KeyState) {
        ...
        self.push_event(KeyboardEvent::Key {
            serial,
            time,
            key,
            state,
    ...
    fn version(&self) -> u32 {
        ...
        KnownKbds::for_each_focused_kbd(&*keyboards, &self.focused_surface, |k| {
            v = Some(k.version());
        });
    ...
    D: 'static,
{
    fn request(
        ...
        let Some(filter) = (AsRef::<dyn WlKeyboardApi + Send + Sync>::as_ref(filter) as &dyn WlKeyboardApi)
            ...
            Request::Filter { serial, action } => {
                ...
                while let Some(e) = events.pop_back() {
                    ...
                    match (action, e.1.is_filterable()) {
                        ...
                        (Action::Passthrough, _) => filter.apply(e),
                    }
                    if stop {
                        return;
                    }

use crate::{
    ...
    wayland::{
        ...
        output::{Output, Inner as OutputInner},
    },
    utils::{
        Rectangle, Point, Logical,
...
use std::{
    ...
    hash::{Hash, Hasher},
    rc::Rc,
    sync::{Mutex, Arc, Weak, atomic::{AtomicUsize, Ordering}},
};
...
pub fn layer_map_for_output(o: &Output) -> RefMut<'_, LayerMap> {
    ...
    userdata.insert_if_missing(|| RefCell::new(LayerMap {
        layers: IndexSet::new(),
        output: weak_output,
        zone: Rectangle::from_loc_and_size((0, 0), o.current_mode()
...
impl LayerMap {
    pub fn map_layer(&mut self, layer: &LayerSurface) -> Result<(), LayerError> {
        if !self.layers.contains(layer) {
            if layer.0.userdata.get::<LayerUserdata>().map(|s| s.borrow().is_some()).unwrap_or(false) {
    ...
    pub fn layers_on(&self, layer: WlrLayer) -> impl DoubleEndedIterator<Item = &LayerSurface> {
        self.layers.iter().filter(move |l| l.layer().map(|l| l == layer).unwrap_or(false))
    }
    ...
    pub fn arrange(&mut self) {
        if let Some(output) = self.output() {
            let output_rect = Rectangle::from_loc_and_size((0, 0), output.current_mode()
                .map(|mode| mode.size.to_logical(output.current_scale()))
                .unwrap_or((0, 0).into())
            );
            let mut zone = output_rect.clone();
            slog::debug!(crate::slog_or_fallback(None), "Arranging layers into {:?}", output_rect.size);
            ...
            for layer in self.layers.iter() {
                ...
                if size.w == 0 { size.w = source.size.w / 2; }
                if size.h == 0 { size.h = source.size.h / 2; }
                if data.anchor.anchored_horizontally() { size.w = source.size.w; }
                if data.anchor.anchored_vertically() { size.h = source.size.h; }
                ...
                if let ExclusiveZone::Exclusive(amount) = data.exclusive_zone {
                    match data.anchor {
                        x if x.contains(Anchor::LEFT) && !x.contains(Anchor::RIGHT) => zone.loc.x += amount as i32 + data.margin.left + data.margin.right,
                        x if x.contains(Anchor::TOP) && !x.contains(Anchor::BOTTOM) => zone.loc.y += amount as i32 + data.margin.top + data.margin.bottom,
                        x if x.contains(Anchor::RIGHT) && !x.contains(Anchor::LEFT) => zone.size.w -= amount as i32 + data.margin.left + data.margin.right,
                        x if x.contains(Anchor::BOTTOM) && !x.contains(Anchor::TOP) => zone.size.h -= amount as i32 + data.margin.top + data.margin.top,
                        _ => {},
                    }
                }

                slog::debug!(crate::slog_or_fallback(None), "Setting layer to pos {:?} and size {:?}", location, size);
                if layer
                    .0.surface
...
impl LayerSurface {
    ...
    pub fn can_receive_keyboard_focus(&self) -> bool {
        self.0.surface.get_surface().map(|surface| {
            with_states(surface, |states| {
                match states.cached_state.current::<LayerSurfaceCachedState>().keyboard_interactivity {
    ...
    pub(super) fn accumulated_damage(&self, for_values: Option<(&Space, &Output)>) -> Vec<Rectangle<i32, Logical>> {
        ...
        if let Some(surface) = self.get_surface() {
            damage.extend(damage_from_surface_tree(surface, (0, 0), for_values).into_iter().flat_map(|rect| rect.intersection(self.bbox())));

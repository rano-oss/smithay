        ...
        R::Frame: 'static,
    {
        ...
        for old_toplevel in state
            ...
            .filter_map(|(id, w)| {
                ...
                && !custom_elements.iter().any(|c| ToplevelId::Custom(c.type_of(), c.id()) == *id)
            {
                    Some(*w)
                } else {
                    None
        ...
        for elem in custom_elements {
            ...
            let old_geo = state.last_state.get(&ToplevelId::Custom(elem.type_of(), elem.id())).cloned();
            ...
            if old_geo.map(|old_geo| old_geo != geo).unwrap_or(false) {
                ...
                let location = geo.loc;
                damage.extend(elem.accumulated_damage(Some((self, output))).into_iter().map(|mut rect| {
                    rect.loc += location;
                    rect
                }));
            }
        ...
        if let Err(err) = renderer.render(
            ...
            |renderer, frame| {
                ...
                for layer in layer_map.layers_on(WlrLayer::Background).chain(layer_map.layers_on(WlrLayer::Bottom)) {
                    ...
                    if damage.iter().any(|geo| lgeo.overlaps(*geo)) {
                        let layer_damage = damage.iter()
                            ...
                            .collect::<Vec<_>>();
                        slog::trace!(self.logger, "Rendering layer at {:?} with damage {:#?}", lgeo, layer_damage);
                        draw_layer(renderer, frame, layer, state.render_scale, lgeo.loc, &layer_damage, &self.logger)?;
                ...
                for window in self.windows.iter() {
                    ...
                    if damage.iter().any(|geo| wgeo.overlaps(*geo)) {
                        ...
                        let win_damage = damage.iter()
                            ...
                            .collect::<Vec<_>>();
                        slog::trace!(self.logger, "Rendering window at {:?} with damage {:#?}", wgeo, win_damage);
                        draw_window(renderer, frame, window, state.render_scale, loc, &win_damage, &self.logger)?;
                        window_state(self.id, window).drawn = true;
                    }
                }
                
                for layer in layer_map.layers_on(WlrLayer::Top).chain(layer_map.layers_on(WlrLayer::Overlay)) {
                    let lgeo = layer_map.layer_geometry(layer);
                    if damage.iter().any(|geo| lgeo.overlaps(*geo)) {
                        let layer_damage = damage.iter()
                            .flat_map(|geo| geo.intersection(lgeo))
                ...
                for elem in custom_elements {
                    ...
                    if damage.iter().any(|geo| egeo.overlaps(*geo)) {
                        let elem_damage = damage.iter()
                            ...
                            .collect::<Vec<_>>();
                        slog::trace!(self.logger, "Rendering custom element at {:?} with damage {:#?}", egeo, elem_damage);
                        elem.draw(renderer, frame, state.render_scale, egeo.loc, &elem_damage, &self.logger)?;
                    }
                }

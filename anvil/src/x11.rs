        ...
        if state.backend_data.render {
            ...
            if let Some(ref surface) = *dnd_guard {
                if surface.as_ref().is_alive() {
                    elements.push(Box::new(draw_dnd_icon(surface.clone(), (x as i32, y as i32), &log)) as Box<dyn RenderElement<_, _, _, _>>);
                }
            }
            ...
            let elements_ref = elements.iter().map(|x| &**x).collect::<Vec<_>>();
            match space.render_output(&mut *renderer, &output, age as usize, CLEAR_COLOR, &elements_ref) {
                Ok(true) => {

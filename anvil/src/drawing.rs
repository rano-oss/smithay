...
use smithay::{
    backend::{
        renderer::{buffer_type, BufferType, Frame, ImportAll, Renderer, Texture, Transform, utils::draw_surface_tree},
        SwapBuffersError,
    },
    desktop::{Space, SurfaceTree, RenderElement},
...
#[cfg(feature = "debug")]
impl<'a, R, F, E, T> RenderElement<R, F, E, T> for FpsElement<T>
where
    R: Renderer<Error = E, TextureId = T, Frame = F> + ImportAll + 'static,
    F: Frame<Error = E, TextureId = T> + 'static,
    E: std::error::Error + Into<SwapBuffersError> + 'static,
    T: Texture + 'static,
{
    ...
    fn geometry(&self) -> Rectangle<i32, Logical> {
        let digits = if self.value < 10 { 1 } else if self.value < 100 { 2 } else { 3 };
    ...
    fn draw(
        ...
        for digit in value_str.chars().map(|d| d.to_digit(10).unwrap()) {
            let damage = damage.iter().flat_map(|x| x.intersection(Rectangle::from_loc_and_size(
                Point::from((offset_x as i32, 0)) + location, (22, 35)
            ))).map(|mut x| {
                x.loc = (0, 0).into();
                x.to_physical(scale.ceil() as i32)
            }).collect::<Vec<_>>();
            frame
                .render_texture_from_to(
                    ...
                    Rectangle::from_loc_and_size(
                        Point::from((offset_x, 0.0)) + location.to_f64().to_physical(scale),
